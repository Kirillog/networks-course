...
2. Почему ICMP-пакет не обладает номерами исходного и конечного портов?
   - Протокол не предназначен для передачи данных, он используется для передачи сообщений об ошибках, других исключительных ситуаций, возникших при передаче данных, и другой оперативной информации.
3. Рассмотрите один из ping-запросов, отправленных вашим хостом. Каковы ICMP-тип и кодовый
   номер этого пакета? Какие еще поля есть в этом ICMP-пакете? Сколько байт приходится на поля
   контрольной суммы, порядкового номера и идентификатора?
   - Тип - 8 (Echo request)
     Кодовый номер - 0
   - Порядковый номер, Контрольная сумма, Идентификатор, Временная метка
   - По 2 на каждое поле, 6 в сумме
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - У первого запроса TTL = 1, у второго TTL = 2, и так далее.
...
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - Поле unused, a в data секции лежит копия всего IPv4 заголовка запроса (включая ICMP и его data секцию)
...
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - Нет копии IPv4 заголовка и дополнительных полей, тип 0 (Echo reply)
   - Типом ICMP-пакета
...
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Наибольшая задержка между маршрутизатором в России и в Стокгольме (5-6)
   - Первый расположен в моей локальной сети, последний -- в Стокгольме
